...
std::pair<Vector2, Vector2> suppress(const Vector2& vA, const Vector2& vA1, const Vector2& vri, const Vector2& vrj) {
    Vector2 vp = vri - vrj;
    double r = vri.x, q = vri.y;
    double p = vp.x, s = vp.y;
...
std::pair<std::vector<Vector2>, int, bool> pbairstow_even(const std::vector<double>& pa, const std::vector<Vector2>& vrs, const Options& options = Options()) {
    int M = vrs.size();
    int N = pa.size() - 1;
...
std::pair<double | std::complex<double>, double | std::complex<double>> find_rootq(const Vector2& vr) {
    double hr = vr.x / 2;
    double d = hr * hr + vr.y;
    if (d < 0) {
